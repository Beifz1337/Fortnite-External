...
#include <random>
#include <format>
#include <D:\walid leaked\ImGui\imgui_internal.h>
#include <D:\walid leaked\ImGui\imgui.h>
...
Vector3 ProjectWorldToScreen(Vector3 WorldLocation) {
	...
	uint64_t chain2 = rpm<uint64_t>(chain1 +  0x140); // W2S
...
void aimbot(float x, float y, float z) {
	...
	if (item.Auto_Fire) {
		mouse_event(MOUSEEVENTF_LEFTDOWN, DWORD(NULL), DWORD(NULL), DWORD(0x0006), ULONG_PTR(NULL)); 
		mouse_event(MOUSEEVENTF_LEFTUP, DWORD(NULL), DWORD(NULL), DWORD(0x0008), ULONG_PTR(NULL)); 
	}
...
void DrawESP() {
	...
	int ActorTeamId = rpm<int>(0xED8); //changes often 4u
	...
	for (unsigned long i = 0; i < entityListCopy.size(); ++i) {
		...
		int MyTeamId = rpm<int>(PlayerState + 0xED8);
		int ActorTeamId = rpm<int>(otherPlayerState + 0xED8);
...
void ToggleButton(const char* str_id, bool* v)
{
	...
	if (ImGui::IsItemHovered())
		col_bg = ImGui::GetColorU32(ImLerp(ImVec4(1.0f, 0.0f, 0.58f, 1.0f), ImVec4(0.0f, 0.0f, 0.80f, 1.0f), t));
	else
		col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.35f, 0.35f, 0.35f, 1.0f), ImVec4(1.0f, 0.0f, 0.58f, 1.00f), t));
...
void render() {
	...
	ImGui::NewFrame();
	ImGui::SetNextWindowSize({ 630, 339 });
	if (item.Show_Menu) {
...
#define TEXT(v) ImVec4(0.860f, 0.930f, 0.890f, v)
		...
		*/
		...
		colors[ImGuiCol_ScrollbarBg] = ImVec4(1.00f, 0.43f, 0.35f, 0.39f);
		colors[ImGuiCol_ScrollbarGrab] = ImVec4(0.20f, 0.25f, 0.29f, 1.00f);
		colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(1.00f, 0.43f, 0.35f, 1.00f);
		colors[ImGuiCol_ScrollbarGrabActive] = ImVec4(1.00f, 0.43f, 0.35f, 1.00f);
		colors[ImGuiCol_CheckMark] = ImVec4(1.0f, 0.0f, 0.58f, 1.00f);
		colors[ImGuiCol_SliderGrab] = ImVec4(1.0f, 0.0f, 0.58f, 1.00f);
		colors[ImGuiCol_SliderGrabActive] = ImVec4(1.0f, 0.0f, 0.58f, 1.00f);
		colors[ImGuiCol_Button] = ImVec4(0.20f, 0.25f, 0.29f, 1.00f);
		colors[ImGuiCol_ButtonHovered] = ImVec4(1.0f, 0.0f, 0.50f, 1.00f);
		colors[ImGuiCol_ButtonActive] = ImVec4(1.0f, 0.0f, 0.50f, 1.00f);
		...
		std::string title = std::format("     Seave#5349    |   {} {}   |     User: {}", __DATE__, __TIME__,  getuser().c_str());
...

		{
			...
			ImGui::SameLine();
			ImGui::SameLine();
			if (ImGui::Button(ICON_FA_CROSSHAIRS " Softaim", ImVec2(140, 35))) fortnitetab = 1;
			ImGui::SameLine();
			if (ImGui::Button(ICON_FA_EYE " Visuals", ImVec2(140, 35))) fortnitetab = 2;
			ImGui::SameLine();
			if (ImGui::Button(ICON_FA_PAPERCLIP " Colors", ImVec2(140, 35))) fortnitetab = 3;
			ImGui::SameLine();
			if (ImGui::Button(ICON_FA_CLOUD " Infos", ImVec2(140, 35))) fortnitetab = 4;
			ImGui::SameLine();
			...
			if (fortnitetab == 1)
			{
				...
				//	ImGui::Checkbox(" AimPrediction", &item.Aim_Prediction); 
...
//
					...
					//ImGui::Checkbox(" Random Hits", &item.Auto_Bone_Switch);
				...
				//ImGui::Selectable("Others Options:"); {
					...
					ImGui::SliderFloat(" Circle FOV", &item.AimFOV, 1, 400);

					if (item.Aim_Speed)
					{
						ImGui::SliderFloat(" Smooth", &item.Aim_Speed, 5, 50);
					}

					ImGui::SliderFloat("Circle FOV Shape", &item.Shape, 7, 50);
			...
			if (fortnitetab == 2)
			{
...

				{
					...
					//ImGui::Combo(xorstr("Box Styles"), &boxmodepos, boxmodes, sizeof(boxmodes) / sizeof(*boxmodes));
					ImGui::Checkbox("2D Box", &item.Esp_box);
					ImGui::SameLine();
					ImGui::SameLine();

					ImGui::Checkbox("3D Box", &item.Esp_3D_Box);
					ImGui::SameLine();
					ImGui::SameLine();
					...
					ToggleButton("Skeleton", &item.skeleton2);
					ImGui::SameLine();
					ImGui::Text("Skeleton");
					...
					ImGui::SliderFloat(" Transparence ESP", &item.Transparency, 0, 50);

					ImGui::SliderFloat(" Epaisseur ESP", &item.Thickness, 1, 20);

					if (item.VisDist)
					{
						ImGui::SliderFloat(" Distance ESP", &item.VisDist, 10, 1000);
					}
			...
			if (fortnitetab == 3)
			{
				...
				ImGui::ColorPicker3(("Box Players"), item.Espbox, ImGuiColorEditFlags_NoInputs);
				ImGui::SameLine();
				ImGui::ColorPicker3(("Corner ESP"), item.BoxCornerESP, ImGuiColorEditFlags_NoInputs);
				ImGui::SameLine();
				ImGui::ColorPicker3(("Lines"), item.LineESP, ImGuiColorEditFlags_NoInputs);
				ImGui::SameLine();
				//ImGui::ColorPicker3(("Triangle ESP"), item.TriangleESP, ImGuiColorEditFlags_NoInputs);
				ImGui::Text("");//seperator;
				ImGui::Text("");//seperator;
				ImGui::ColorPicker3(("Circle FOV"), item.DrawFOVCircle, ImGuiColorEditFlags_NoInputs);
				ImGui::SameLine();
				ImGui::ColorPicker3(("Crosshair"), item.CrossHair, ImGuiColorEditFlags_NoInputs);
				ImGui::SameLine();
				ImGui::ColorPicker3(("line Ver."), item.LockLine, ImGuiColorEditFlags_NoInputs);
				ImGui::SameLine();
				//ImGui::ColorPicker3(("Triangle ESP Filled"), item.TriangleESPFilled, ImGuiColorEditFlags_NoInputs);
				//ImGui::SameLine();
			...
			if (fortnitetab == 4)
			{
				...
				if (ImGui::Button("Join Community"))
				{
					system("start https://discord.gg/fNbZEewUHp");
				}
				...
				ImGui::TextColored(ImVec4(0.28f, 0.56f, 1.00f, 1.00f), "Site:");;
				...
				if (ImGui::Button("Visit"))
				{
					system("start https://www.youtube.com/channel/UCihVE4aXvv9RWKm_mYE0fdg");
				}


				ImGui::Text("      ");

				ImGui::TextColored(ImVec4(0.28f, 0.56f, 1.00f, 1.00f), "TikTok:");;
				if (ImGui::Button("Visit"))
				{
					system("start https://www.youtube.com/watch?v=DLzxrzFCyOs");
				}
			...
			ImGui::Separator();
				...
				ImGui::Text("Status:");
				ImGui::SameLine();
				ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.58f, 1.00f), "Undetected");
				ImGui::SameLine();
				ImGui::Separator();
				ImGui::Text("Dev:");
				ImGui::SameLine();
				ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.58f, 1.00f), "Seave#5349");

				//	if (ImGui::Button("Discord", ImVec2(120, 20)));
				// {
	

				//system("start https://discord.gg/p8wJgkzD");
						//}
